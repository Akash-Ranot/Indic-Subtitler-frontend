...
export default function Footer() {
  const navigation = ["Product", "Features", "Blog"];
  const legal = ["Terms", "Privacy", "Legal"];

  return (
    <div className="relative" style={{ backgroundColor: '#F0F0F0' }}>
      <Container>
        <div className="grid max-w-screen-xl grid-cols-1 gap-10 pt-10 mx-auto mt-5 border-t border-gray-100 dark:border-trueGray-700 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <div>
              <Link href="/" className="flex items-center space-x-2 text-2xl font-medium text-indigo-500 dark:text-gray-100">
                <span style={{ padding: '0.5rem' }}>Indic-Subtitler</span>
              </Link>
            </div>
          ...
          <div className="">
            ...
            <div className="flex mt-5 space-x-5 text-gray-400 dark:text-gray-500">
              {/* ... (similar changes for social media icons) */}
            </div>
          </div>
        </div>

        <div className="my-10 text-sm text-center text-gray-600 dark:text-gray-400">
          Copyright © {new Date().getFullYear()}. Made By AR
        </div>
      </Container>
    </div>
  );
